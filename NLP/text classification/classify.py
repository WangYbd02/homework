...
if __name__ == '__main__':
    ...
    for i, filename in enumerate(files):  # i是标签
    ...
    """--------------------------------------------------------------------------"""
    """2.转化为词向量"""
    tv = TfIdfVectorizer()  # 实例化基于tf-idf的词向量转化对象
    x_train_tf = tv.fit_transform(train_text_list)  # 训练集文本转化为词向量
    x_test_tf = tv.transform(test_text_list)  # 测试集文本转化为词向量
    """--------------------------------------------------------------------------"""
    """3.训练分类器"""
    print('start training.')
    # clf = MultinomialNaiveBayes(if_ratio=False)  # 多项式朴素贝叶斯
    clf = KNeighbors(n_neighbors=11)  # K近邻分类器
    clf.fit(x_train_tf, train_label_list)  # 训练分类器
    print('training finish.')
    """--------------------------------------------------------------------------"""
    """4.测试并输出结果"""
    print('waiting test...')
    y_predicted = clf.predict(x_test_tf)  # 分类器预测结果
